...
import (
	...
	"pressure-test-admin/tool/go-stress-testing/tools"
...
func table(successNum, failureNum uint64, errCode *sync.Map,
	...
	result := fmt.Sprintf("%4.0fs│%7d│%7d│%7d│%8.2f│%8.2f│%8.2f│%8.2f│%8s│%8s│%v",
		...
		printMap(errCode))
	fmt.Println(result)
	res, _ := json.Marshal(schema.PublicRsp{
		Code:    200,
		Message: "OK",
		Data: struct {
			...
			Fail:           int64(failureNum),
			Qps:            fmt.Sprintf("%8.2f", qps),
			MaxTime:        fmt.Sprintf("%8.2f", maxTimeFloat),
			MinTime:        fmt.Sprintf("%8.2f", minTimeFloat),
			AvgTime:        fmt.Sprintf("%8.2f", averageTime),
			CodeInfo:       printMap(errCode),
		},
	})
